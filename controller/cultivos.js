...
ruta.get('/cultivos/activos/', (req, res) => {
  let sql = "SELECT * FROM cultivos WHERE EstLogico = 1 ORDER BY id_cultivo DESC";
...
ruta.get('/cultivos/activos/:id', (req, res) => {
  conexion.query("SELECT * FROM cultivos WHERE id_cultivo = ? AND EstLogico = 1", [req.params.id], (err, rows) => {
    if (err) throw err;
    res.json(rows);
  });
});
...
ruta.get('/cultivos/:id', (req, res) => {
  conexion.query("SELECT * FROM cultivos WHERE id_cultivo = ?", [req.params.id], (err, rows) => {
    if (err) throw err;
    res.json(rows);
  });
});
...
ruta.put('/cultivos/', function(req, res) {
  let sql = "UPDATE cultivos SET id_usuario = ?, tipo_cultivo = ?, fecha_siembra = ?, fecha_cosecha = ?, estado = ? WHERE id_cultivo = ? AND EstLogico = 1";
  console.log('Parámetros:', [req.body.id_usuario, req.body.tipo_cultivo, req.body.fecha_siembra, req.body.fecha_cosecha, req.body.estado, req.body.id_cultivo]);
  conexion.query(sql, [req.body.id_usuario, req.body.tipo_cultivo, req.body.fecha_siembra, req.body.fecha_cosecha,  req.body.estado, req.body.id_cultivo], function (error, results) {
    if (error) throw error;
    if (results.affectedRows) {
