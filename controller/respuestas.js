...
ruta.get('/respuestas/activos/', (req, res) => {
  let sql = "SELECT * FROM respuestas WHERE EstLogico = 1 ORDER BY id_respuesta DESC";
...
ruta.get('/respuestas/activos/:id', (req, res) => {
  conexion.query("SELECT * FROM respuestas WHERE id_respuesta = ? AND EstLogico = 1", [req.params.id], (err, rows) => {
    if (err) throw err;
    res.json(rows);
  });
});
...
ruta.get('/respuestas/:id', (req, res) => {
  conexion.query("SELECT * FROM respuestas WHERE id_respuesta = ?", [req.params.id], (err, rows) => {
    if (err) throw err;
    res.json(rows);
  });
});
...
ruta.put('/respuestas/:id', (req, res) => {
  conexion.query("UPDATE respuestas SET EstLogico = 0 WHERE id_respuesta = ?", [req.params.id], (err, results) => {
    if (err) throw err;
...
ruta.post('/respuestas/', (req, res) => {
  let sql = "INSERT INTO respuestas SET ?";
  let data = {
...
ruta.put('/respuestas/', (req, res) => {
  let sql = "UPDATE respuestas SET id_publicacion = ?, id_usuario = ?, contenido = ?, fecha_respuesta = ? WHERE id_respuesta = ? AND EstLogico = 1";
  conexion.query(sql, [req.body.id_publicacion, req.body.id_usuario, req.body.contenido, req.body.fecha_respuesta, req.body.id_respuesta], (error, results) => {
    if (error) throw error;
    if (results.affectedRows)
